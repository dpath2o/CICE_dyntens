              ...
              if ((aicen(n) > puny) .and. (SUM(afsd_init(:)) > puny) &
                 ...
                 afsd_tmp =  afsd_init
                  ...
                  DO WHILE (elapsed_t < dt)
                     ...
                     d_afsd_tmp = get_dafsd_wave( &
                          afsd_tmp, fracture_hist, frac)

                     ! Warn once if convergence is already unusually slow.
                     if (nsubt == 101) then

                        write(warnstr,*) subname, &
                             ' warning: step_wavefracture struggling', &
                             ' to converge; nsubt=',nsubt
                     ...
                     !--------------------------------------------------
                     if (.not. ieee_is_finite(subdt) .or. &
                         subdt <= c0 .or. &
                         elapsed_t + subdt <= elapsed_t) then

                        write(warnstr,*) subname, &
                             ' FSD subdt failure: nsubt=',nsubt, &
                             ' elapsed=',elapsed_t, &
                             ' dt=',dt, &
                             ' subdt=',subdt
                        ...
                        write(warnstr,*) subname, &
                             ' afsd min/max=', &
                             MINVAL(afsd_tmp),MAXVAL(afsd_tmp), &
                             ' dafsd min/max=', &
                             MINVAL(d_afsd_tmp),MAXVAL(d_afsd_tmp)
                        ...
                        write(warnstr,*) subname, &
                             ' thickness category n=',n, &
                             ' sum_afsd=',SUM(afsd_tmp), &
                             ' sum_dafsd=',SUM(d_afsd_tmp)
                     ...
                     !--------------------------------------------------
                     ! Hard upper limit: never permit an effectively
                     ! unbounded adaptive integration.
                     !--------------------------------------------------
                     if (nsubt >= max_wave_subcycles) then

                        write(warnstr,*) subname, &
                             ' FSD subcycle limit: nsubt=',nsubt, &
                             ' elapsed=',elapsed_t, &
                             ' dt=',dt, &
                             ' subdt=',subdt
