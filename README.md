...
The September control completed a two-day run; see the [control audit](docs/development/stage0-dyntens01.md) and [baseline record](docs/development/baseline-run.md). Numerical and split-run restart checks remain open. [Stage 1: local coefficient with g=1](docs/development/stage1-g1.md) is implemented behind the disabled-by-default `use_dyntens` switch. Rebuild on Gadi and compare the original control, rebuilt disabled path and enabled g=1 path before adding FSD feedback.
